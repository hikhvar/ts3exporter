...
func NewServerInfo(c serverquery.Executor) *ServerInfo {
	return &ServerInfo{
		...
		MaxDownloadTotalBandwidth:      prometheus.NewDesc(fqdn(serverInfoSubsystem, "max_download_bandwidth"), "maximal bandwidth available for downloads", serverInfoLabels, nil),
		MaxUploadTotalBandwidth:        prometheus.NewDesc(fqdn(serverInfoSubsystem, "max_upload_bandwidth"), "maximal bandwidth available for uploads", serverInfoLabels, nil),
